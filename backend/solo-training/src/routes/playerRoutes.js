...
// Ruta para insertar un nuevo jugador (ahora recibe userId como parámetro)
fastify.post('/players/:userId', { schema: playerSchema, preValidation: [fastify.authenticate] }, async (request, reply) => {
  try {
    const { userId } = request.params; // Obtener el ID del usuario desde los parámetros
    ...
    // Actualizar el usuario con el nuevo jugadorId
    const updateResult = await db.collection("users").updateOne(
      { _id: new ObjectId(userId) },
      { $set: { jugadorId: result.insertedId } }
    );
